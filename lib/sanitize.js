...
export function sanitizeUrl(url) {
    ...
    if (!/^(https?|tel|mailto|#|\/)/i.test(trimmed)) {
        // basic check to avoid prepending https:// to random words, but for a URL field it's mostly safe
        trimmed = 'https://' + trimmed;
    }

    // Double check safe protocols
    ...
    if (typeof data === 'object') {
        ...
        for (const [key, val] of Object.entries(data)) {
            ...
            if (key === 'url' || key === 'href' || key === 'src') {
                ...
                // Only sanitize iconUrl if it has a real value; otherwise skip to avoid '#' as broken img src
                if (val && typeof val === 'string' && val.startsWith('http')) {
                    clean[key] = sanitizeUrl(val);
                } else if (val) {
                    clean[key] = val; // keep as is if it's some other format
                }
                // If falsy, don't include iconUrl at all (let it be undefined)
            } else {
